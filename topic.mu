...
status = "`F844[✖ closed]`f " if topic["is_closed"] else ""
print(f"`F666{forum.fmt_time(topic['created_at'])}  ·  by `f{a_link}  {status}")
print()
...
if tags:
    ...
    for i, tag in enumerate(tags):
        ...
        if token:
            t_f += f"|session={token}"
        print(f"`Fa60`[{tag}`{forum.page_path}/tag.mu`{t_f}]`f", end="")
    print()
    print()
...
if comments:
    ...
    for i, c in enumerate(comments):
        ...
        print(f"`F555{forum.fmt_time(c['created_at'])}  ·  `f{a_link}")
        print(c["body"])
        if user and (user.get("is_admin") or user.get("is_mod") or user["id"] == c["author_id"]):
            dc_f = f"comment={c['id']}"
            if token:
                dc_f += f"|session={token}"
            print(forum.btn("✖ Delete", f"{forum.page_path}/admin/delete_comment.mu", dc_f, "danger"))
        print()
...
if user and can_write and not topic["is_closed"]:
    ...
    print(forum.btn("↩ Post Reply", f"{forum.page_path}/topic.mu", f"topic={topic_id}|action=comment|session={token}|*"))
elif user and not can_write:
    print("`F777You do not have permission to post in this subforum.`f")
elif not user:
    print(f"`Fa60`[→ Login to reply`{forum.page_path}/login.mu]`f")
elif topic["is_closed"]:
    print("`F844This topic is closed.`f")
